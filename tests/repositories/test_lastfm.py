...
class TestSaveLastfmProfile:
    ...
    def test_updates_existing_profile(self, repo_session, seed_user):
        ...
        profiles = repo_session.query(LastfmProfile).filter_by(user_id=seed_user.id).all()
...
class TestUpsertTopArtists:
    def test_inserts_artists(self, repo_session, seed_user):
        repo = LastfmRepository()
        artists = [
            {"name": "Yo La Tengo", "mbid": "3121f5a6-0854-4a15-a3f3-4bd359073857", "playcount": "500", "@attr": {"rank": "1"}},
            {"name": "Sonic Youth", "mbid": "", "playcount": "300", "@attr": {"rank": "2"}},
        ]
        repo.upsert_top_artists(repo_session, seed_user.id, "lastfm", "overall", artists)
        results = repo_session.query(TasteProfileArtist).filter_by(user_id=seed_user.id).all()
    ...
    def test_updates_playcount_on_resync(self, repo_session, seed_user):
        repo = LastfmRepository()
        artists_v1 = [
            {"name": "Yo La Tengo", "mbid": "", "playcount": "500", "@attr": {"rank": "1"}},
        ]
        repo.upsert_top_artists(repo_session, seed_user.id, "lastfm", "overall", artists_v1)
        artists_v2 = [
            {"name": "Yo La Tengo", "mbid": "", "playcount": "600", "@attr": {"rank": "1"}},
        ]
        repo.upsert_top_artists(repo_session, seed_user.id, "lastfm", "overall", artists_v2)
        results = repo_session.query(TasteProfileArtist).filter_by(user_id=seed_user.id).all()
...
class TestUpsertTopAlbums:
        ...
        albums = [
            {
                ...
                "playcount": "200",
                "artist": {"name": "Yo La Tengo", "mbid": "3121f5a6-0854-4a15-a3f3-4bd359073857"},
                "@attr": {"rank": "1"},
            }
        ]
        repo.upsert_top_albums(repo_session, seed_user.id, "lastfm", "overall", albums)
        results = repo_session.query(TasteProfileAlbum).filter_by(user_id=seed_user.id).all()
        assert len(results) == 1
    ...
    def test_deletes_stale_albums(self, repo_session, seed_user):
        repo = LastfmRepository()
        albums_v1 = [
            {"name": "Album1", "mbid": "", "playcount": "100", "artist": {"name": "A1", "mbid": ""}, "@attr": {"rank": "1"}},
            {"name": "Album2", "mbid": "", "playcount": "50", "artist": {"name": "A2", "mbid": ""}, "@attr": {"rank": "2"}},
        ]
        repo.upsert_top_albums(repo_session, seed_user.id, "lastfm", "overall", albums_v1)
        albums_v2 = [
            {"name": "Album1", "mbid": "", "playcount": "150", "artist": {"name": "A1", "mbid": ""}, "@attr": {"rank": "1"}},
        ]
        repo.upsert_top_albums(repo_session, seed_user.id, "lastfm", "overall", albums_v2)
        results = repo_session.query(TasteProfileAlbum).filter_by(user_id=seed_user.id).all()
        assert len(results) == 1
